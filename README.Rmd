...
- **`palette_demo()`** creates an exploratory base R graph from a vector of hexadecimal codes (like that returned by either `palette_extract()` or `palette_sort()`) and provides an option to export that plot if desired

- **`palette_ggdemo()`** creates an exploratory `ggplot2` graph from a vector of hexadecimal codes (like that returned by either `palette_extract()` or `palette_sort()`)
