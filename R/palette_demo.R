...
#' 
#' @description Accepts the hexadecimal code vector returned by `palette_extract()` or `palette_sort()` and creates a base plot of all returned colors labeled with their HEX codes. This will facilitate (hopefully) your selection of which of the 25 colors you would like to use in a given context.
#' 
#' @param palette Vector of hexadecimal codes like that returned by `palette_extract()` or `palette_sort()`
